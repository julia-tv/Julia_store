...
function addToCartByData(element, keyPath) {
  const parts = keyPath.split('.');
  let data = window.PRODUCTS;
  
  // Йдемо по шляху (наприклад, bundles -> epherium12)
  parts.forEach(p => { 
    if(data) data = data[p]; 
  });
  ...
  if (data) {
    ...
    // Якщо це об'єкт (як epherium або bundles.epherium12)
    if (typeof data === 'object' && data !== null) {
      // Назва: або з поля title, або генеруємо за ключем
      title = data.title;
      if (!title) {
        if (keyPath.includes('epherium12')) title = "Ефіріум (12 шт)";
        else if (keyPath.includes('epherium3')) title = "Ефіріум (3 шт)";
        else title = "Набір";
      }
      price = data.price;
    } else {
      // Якщо шлях веде прямо до числа (наприклад, epherium.price)
      title = "Товар"; 
      price = data;
    }
    ...
    console.error("Дані не знайдені за шляхом:", keyPath);
  }
}
